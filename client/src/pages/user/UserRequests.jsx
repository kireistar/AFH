...
const UserRequests = ({ onOpenRequestModal }) => {
  const myRequests = [
    { id: 'REQ-012', asset: 'MacBook Pro M2', date: 'Oct 25, 2023', urgency: 'Low', status: 'Approved' },
    { id: 'REQ-013', asset: 'iPad Pro', date: 'Oct 28, 2023', urgency: 'High', status: 'Pending Manager' }
  ];
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
      <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
        ...
        <button 
          ...
          className="px-4 py-2 bg-[#1E3A8A] text-white rounded-xl text-sm font-semibold hover:bg-blue-900 transition-colors shadow-sm"
        >
          + New Request
        </button>
      </div>
              ...
              <tr key={req.id} className="hover:bg-slate-50/50 transition-colors">
                ...
                <td className="p-4 text-sm">
                  <span className={`px-2.5 py-1 rounded-full text-xs font-bold border ${
                    req.status === 'Approved' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-yellow-50 text-yellow-700 border-yellow-200'
                  }`}>{req.status}</span>
                </td>
              </tr>
            ))}
